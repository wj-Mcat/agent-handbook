...
大语言模型在预训练阶段包含的内容相当丰富且复杂，主要围绕大规模无标签文本数据的处理和模型训练展开，当然此篇内容主要围绕 <Hl color="green">decoder-only</Hl> 类别模型围绕展开介绍。

> 更多的模型架构请查看 [模型的主体架构类型](#主体架构类型)。
...
当 [**数据都准备好了**](./01-dataset.mdx)，就可以开始训练模型了，此时需要考虑主体模型架构、预训练目标函数、训练配置等等。
...
![moe structures](./imgs/moe-structures.png)

<center>图片来源于：[A Survey on Mixture of Experts <sup>20</sup>](#moe-structures)</center>

### 不同模型架构的影响

> 以下仅为个人观点，绝大部分认知来源于**自我思考**。

从 **Llama 1-3** 可见，模型结构对于模型性能的影响微乎其微，更重要的数据质量和数据分布。（当然就目前看来数据质量更为重要）
...
* **Encoder-Decoder 架构**
   ...
   1. 机器翻译：将一种语言的序列翻译成另一种语言。
   2. 文本摘要：将长文本序列压缩成简洁的摘要。
   3. 对话系统：根据输入的对话内容生成响应。
   4. 语音识别：将语音信号转换为文本序列。
   5. 图像描述生成：将图像内容转换为自然语言描述。

   可是此类模型有很严重的缺点：
   1. 训练效率：由于需要处理序列数据，训练过程可能比较慢。
   2. 长序列处理：对于非常长的序列，模型可能难以捕捉长距离依赖。
   3. 注意力机制需求：没有注意力机制的简单Encoder-Decoder模型在长序列上表现不佳，需要引入额外的注意力机制。
   4. 资源消耗：模型可能需要大量的计算资源和内存来存储中间状态和参数。
...
:::tip 由于文章篇幅有限，此处就不做过多的详细解释，后续会给每个子章节补充详细内容。
:::

## Scaling Laws

大语言模型预训练中的 Scaling Law 是一个描述系统性能如何随着模型规模、训练数据量以及计算资源等因素的增加而变化的规律，Scaling Law是一种数学表达，它揭示了系统性能与其规模（如参数量、训练数据量、计算量等）之间的幂律关系
...
* [20] [A Survey on Mixture of Experts](https://arxiv.org/pdf/2407.06204) <div id="moe-structures" />